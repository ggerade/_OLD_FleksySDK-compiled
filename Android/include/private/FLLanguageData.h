...
public:
  ...
  void setPunctuation(const std::vector<FLUnicodeString> &p) { _punctuation = p; }
  const std::vector<FLUnicodeString> &getPunctuation() const { return _punctuation; }
  
  void setFullStops(const std::vector<FLUnicodeString> &f) { _fullStops = f; }
  const std::vector<FLUnicodeString> &getFullStops() const { return _fullStops; }
