...
namespace EncryptionUtilities {
  ...
  void* encrypt(void* data, size_t dataLength, size_t& outLength, const char* key = DEFAULT_M_VALUE);
  ...
  void* decrypt(void* data, size_t dataLength, size_t& outLength, const char* key = DEFAULT_M_VALUE);

#if USE_APPLE_CRYPTO
  size_t appleCrypt(void* data, size_t dataLength, bool encrypt, const char* key = DEFAULT_M_VALUE);
#endif
